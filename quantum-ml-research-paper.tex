...
The MNIST database of handwritten digits~(\cite{mnist-digits}) was used as data set (Figure~\ref{fig:mnist}) to demonstrate its ability of doing classification.
...
The data of the functions are one-dimensional, and we employ a data re-uploading technique described by \textcite{perez2019data} to achieve generalization with a small amount of qubits (Figure~\ref{fig:data-reuploading-circuit}).
\begin{figure}[ht]
	\[
	\Large
	...
	\]
	\caption{Data re-uploading QNN circuit used to achieve generalization with a small amount of qubits. $U_\ell$ represents a layer as before in Section~\ref{sec:bdc}, and $U_x$ encodes the data into a quantum state. In our example we use one-dimensional data, but $U_x$ can be extended to encode a high-dimensional vector.}
...
The state space of a quantum system grows exponentially with the amount of particles, making simulation of higher amount of particles intractable on a classical computer.
