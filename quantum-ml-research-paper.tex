...
The absolute square $|c_0|^2$ gives us the probability of finding the particle in state \ket{0}, and $|c_1|^2$ gives us the probability of finding the particle in state \ket{1}.
...
These quantum annealers are restricted to solving optimization problems, of which its promised speedup is not widely agreed upon~(\cite{how-quantum-dwave, aaronson-dwave, detecting-quantum-speedup}).
...
\begin{enumerate}
	...
	\item Apply $L$ layers of parameterized unitaries: $\mathcal{U}(\vec{\theta})$\ket{x}, where $\mathcal{U}(\vec{\theta}) = U(\vec{\theta}_L) U(\vec{\theta}_{L-1}) \ldots U(\vec{\theta}_1)$.
	\item Measure expectation value $\expval{Z} = \bra{x}\mathcal{U}(\vec{\theta})^\dagger Z\, \mathcal{U}(\vec{\theta})\ket{x}$ of last qubit.
...
We predict the QNN's performance can be improved with more epochs, redundantly encoding data and by using data re-uploading techniques as suggested by~\textcite{vidal2019input}~and~\textcite{perez2019data}.
...
A data point $(\begin{matrix}x_1 & x_2\end{matrix})$ is encoded as $\prod_{i=0}^{n}R_x^{(i)}(x_1)R_y^{(i)}(x_2)$.
...
We see again that the QNN has an easy time learning circular functions due to the encoding method.
Other functions are approximated well, but leaves room for improvement.
Possible improvement can be made by employing better encoding techniques.
...
\begin{figure}[ht]
	...
	\caption{Performance of our QNN learning continuous functions. The QNNs were trained for 100 epochs with 3 layers. RMSprop was used as optimizer with learning rate 0.02, momentum 0.8, and smoothing constant 0.999. Input data was encoded redundantly four times, resulting in five qubits total. The blue points represent the teacher data shown to the network, and the red line represents the learned function.}
