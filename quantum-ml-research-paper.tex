...
Adaptive learning rate methods such as RMSprop and Adam convergence faster and find a better optimum compared to vanilla SGD.
...
We do further experiments with the QNN in the appendices.
...
Appendix~\ref{sec:qnn-regression} demonstrates the QNN's capability of doing regression tasks.
...
Furthermore, we use the same encoding techniques described in Appendix~\ref{sec:classification-other}.
We take on the problem of learning the functions $-x^2$, $\cos x$, $|x|$ and $e^x$ to demonstrate its ability of representing non-linear functions.
...
\begin{figure}[ht]
	...
	\caption{Performance of our QNN learning real functions. The QNNs were trained for 100 epochs with 2 layers and RMSprop as optimizer with learning rate 0.02. Input data was encoded redundantly 3 times, resulting in 4 qubits used total.}
