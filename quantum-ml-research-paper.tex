...
\begin{abstract}
Quantum computing is a promising technology of the near future, shown to speed up certain computational problems.
...
Furthermore, quantum supremacy---which is the potential ability of a quantum computer to solve problems that classical computers practically cannot efficiently~(\cite{preskill-qc})---has not been achieved yet as of writing this.
...
In the years since, HHL has been proposed for multiple machine learning applications,\footnote{HHL and the algorithms which use it can only provide an exponential speedup by overcoming some major caveats~(\cite{aaronson2015read}). For example, the problem of loading a large amount of classical data into a quantum computer can quickly diminish or destroy the potential quantum speedup.} such as $k$-means clustering~(\cite{lloyd2013quantum}), linear regression~(\cite{schuld2016prediction, yu2017improved, wang2017quantum}), and support vector machines~(\cite{rebentrost2014quantum}).
...
\begin{enumerate}
	...
	\end{equation}
	\item Apply $L$ layers of parameterized unitaries: $\mathcal{U}(\vec{\theta})\ket{x}$, where
	\begin{equation}
	\mathcal{U}(\vec{\theta}) = U(\vec{\theta}_L) U(\vec{\theta}_{L-1}) \ldots U(\vec{\theta}_1).
	\end{equation}
	\item Measure expectation value
	\begin{equation}
	\expval{\sigma_z} = \bra{x}\mathcal{U}(\vec{\theta})^\dagger \sigma_z\, \mathcal{U}(\vec{\theta})\ket{x}
	\end{equation}
	of last qubit.
...
The network was shown the complete training data set consisting of 36.012 samples once and tested on 4506 samples.
...
Though this is just one small step to advance QML and make QML more accessible to researchers around the world, every little step counts.
It is likely that most QML approaches will not be of much practical use in the near future.
Variational algorithms however might already provide a quantum advantage at the level of 50-100 qubits for certain problems, but we should not expect the claimed exponential speedups to be achieved on near-term devices.
There is still much work to be done in implementation and theory, but the work done so far is promising.
Quantum machine learning may very well solve the computational problems we are facing today and revolutionize the field of machine learning.
