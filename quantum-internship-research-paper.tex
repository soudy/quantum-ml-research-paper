...
As we are dealing with probabilities, the quantum state should be normalized: $\sum_{i}|c_i|^2 = 1$.
...
\end{equation}
where $D$ is the number of training samples, $\ket{y_d}$ the correct label state of input $\ket{x_d}$, $\vec{\theta}$ are the circuit parameters and $\mathcal{U}(\vec{\theta})$ the parameterized circuit unitaries.
