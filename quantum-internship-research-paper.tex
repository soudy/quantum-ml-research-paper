...
Quantum machine learning can help us process the huge amounts of data we keep creating, due to their ability to encode exponentially more data in their state compared to classical bits.
...
To realize these applications we need to overcome some major obstacles.
...
A quantum random access memory (qRAM) architecture has been proposed for this~(\cite{qram}), but it largely depends on the data being uniform, which our data often is not.
...
This implies we can't efficiently retrieve output data stored in an exponentially large Hilbert space.
...
%\begin{equation}
%F(\vec{\theta}, \vec{x}) = \lvert\bra{\psi_c}U(\vec{\theta})\ket{x, 1}\rvert^2.
%\end{equation}
