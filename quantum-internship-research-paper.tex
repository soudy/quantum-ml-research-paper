...
\begin{figure}[ht]
	\[
	\Large
	\Qcircuit @C=0.7em @R=0.25em @!R {
		& & \multigate{5}{U(\vec{\theta})} & \qw & & \gate{R_x(\vec{\theta}_{1, 1})} & \gate{R_y(\vec{\theta}_{1, 2})} & \gate{R_z(\vec{\theta}_{1, 3})} & \ctrl{1} & \qw & \qw & \ctrl{5} & \qw \\
		& & \ghost{U(\vec{\theta})} & \qw & & \gate{R_x(\vec{\theta}_{2, 1})} & \gate{R_y(\vec{\theta}_{2, 2})} & \gate{R_z(\vec{\theta}_{2, 3})} & \targ & \ctrl{1} & \qw & \qw & \qw \\
		& & \ghost{U(\vec{\theta})} & \qw & & \gate{R_x(\vec{\theta}_{3, 1})} & \gate{R_y(\vec{\theta}_{3, 2})} & \gate{R_z(\vec{\theta}_{3, 3})} & \qw & \targ & \qw & \qw & \qw  \\
		& & & ~~= & & \vdots & \vdots & \vdots & & \vdots & & & & & \\
		& & \ghost{U(\vec{\theta})} & \qw & & \gate{R_x(\vec{\theta}_{n-1, 1})} & \gate{R_y(\vec{\theta}_{n-1, 2})} & \gate{R_z(\vec{\theta}_{n-1, 3})} & \qw & \qw & \ctrl{1} & \qw & \qw \\
		& & \ghost{U(\vec{\theta})} & \qw & & \gate{R_x(\vec{\theta}_{n, 1})} & \gate{R_y(\vec{\theta}_{n, 2})} & \gate{R_z(\vec{\theta}_{n, 3})} & \qw & \qw & \targ & \targ & \qw
	}
	\]
...
It shows the potential, given we solve the issues surrounding qRAM~(\cite{aaronson2015read}), to use quantum computers to process (quantum) data which classical computers cannot process.
\begin{figure}[ht]
	\centering
...
To encode a $28 \times 28$-dimensional image we need $\lceil \log_2(28 \cdot 28) \rceil = 10$ qubits, plus a readout qubit, so $11$ qubits total.
\begin{figure}[ht]
	\centering
