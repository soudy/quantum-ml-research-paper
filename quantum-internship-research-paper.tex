...
\end{equation}
Quantum states are denoted using Dirac notation \ket{\,\cdotp\,} (named a \emph{ket}), which is a column vector in $\mathbb{C}^n$.
Here the coefficients $\{c_0, c_1\}$ are complex valued numbers called \emph{probability amplitudes}.
The computational basis states $\{\ket{0}, \ket{1}\}$ are defined as the column vectors $\begin{pmatrix}1 & 0\end{pmatrix}^T$ and $\begin{pmatrix}0 & 1\end{pmatrix}^T$ respectively.
...
\end{equation}
where the \emph{bra} notation $\bra{\varphi}$ represents the conjugate transpose of $\ket{\varphi}$ ($\bra{\varphi} = \ket{\varphi}^\dagger$).
...
\begin{equation}
\begin{aligned}
\ip{0}{\psi} &= \bra{0}\dfrac{i}{2}\ket{0} - \bra{0}\dfrac{\sqrt3}{2}\ket{1} \\
&= \dfrac{i}{2}\ip{0}{0} - \dfrac{\sqrt3}{2}\ip{0}{1} \\
&= \dfrac{i}{2}\begin{pmatrix}1 & 0\end{pmatrix} \qstatezero - \dfrac{\sqrt3}{2}\begin{pmatrix}1 & 0\end{pmatrix} \qstateone \\
&= \dfrac{i}{2}. \\
\end{aligned}
\end{equation}
Then we take the square norm to give $\lvert \ip{0}{\psi} \rvert^2 = \lvert i/2 \rvert^2 = 1/4$, yielding the result we expected.
...
%The orthogonality of the computational basis states can be understood from this: $\ip{0}{0} = 1$ and $\ip{0}{1} = 0$.
...
\begin{equation}
\mathcal{F}(\vec{\theta}, \vec{x}) = \dfrac{1}{M} \sum_{i=0}^{M} \left(1 - \lvert \bra{y_i}U(\vec{\theta})\ket{x_i, 1} \rvert^2\right),
\end{equation}
where $\vec{x}$ is a set of $M$ training samples, $\ket{y_i}$ the correct label state of sample $\vec{x}_i$ and $\vec{\theta}$ are the circuit parameters.
...
\begin{table}[ht]
	...
	\begin{minipage}{.485\textwidth}
		...
		\begin{tabular}{ c|c|c } 
			...
			\hline
			\multirow{3}{4em}{SGD} & 4 & 0\% \\ 
			& 8 & 0\% \\ 
			& 10 & 0\% \\
			& 12 & 0\% \\
			\hline
			\multirow{3}{4em}{RMSprop} & 4 & 0\% \\ 
			& 8 & 0\% \\ 
			& 10 & 0\% \\
			& 12 & 0\% \\
			\hline
			\multirow{3}{4em}{Adam} & 4 & 0\% \\ 
			& 8 & 0\% \\ 
			& 10 & 0\% \\
			& 12 & 0\% \\
			\hline
