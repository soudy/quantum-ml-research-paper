...
\begin{equation}
\mathcal{F}(\vec{\theta}, \vec{x}) = \dfrac{1}{M} \sum_{i=0}^{M} \left(1 - \lvert \bra{s_i}U(\vec{\theta})\ket{x_i, 1} \rvert^2\right),
\end{equation}
where $\vec{x}$ is a set of $M$ training samples, $\ket{s_i}$ the correct label state of a sample $\vec{x_i}$ and $\vec{\theta}$ are the circuit parameters.
...
\begin{figure}[ht]
	...
	\end{blochsphere}
	\caption{Bloch sphere representation of six maximally orthogonal points which are used as labels. The circuit output state \ket{\psi} would get classified to the class corresponding to the label state \ket{0} as their fidelity $\lvert\ip{0}{\psi}\rvert^2$ is the largest.}
